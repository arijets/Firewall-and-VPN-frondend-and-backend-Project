			...
			<div class="row">
				<div class="<?php bc('5', '5', '', ''); ?>">
					...
					if ( !empty( $theme_options['top_header_text'] ) ) {
						?>
						<p><?php echo esc_html( $theme_options['top_header_text'] ); ?></p>
						<?php
					}
					?>
				</div>
					...
					<div class="<?php bc('7', '7', '', ''); ?> text-right">
								...
								if( !empty( $languages)  ){
									...
									foreach( $languages as $l ){
										...
										if( $l['country_flag_url'] ){
											if( !$l['active'] ) echo '<a href="' . esc_url( $l['url'] ) . '" title="' . esc_html( $l['translated_name'] ) . '">';
											echo '<img src="' . esc_url( $l['country_flag_url'] ) . '" height="12" alt="' . esc_html( $l['translated_name'] ) . '" width="18" />';
											if( !$l['active'] ) echo '</a>';
										}
										echo '</li>';
									}
						...
						<p>
							...
							if ( isset( $theme_options[ 'header_opening_hours' ] ) && ! empty( $theme_options[ 'header_opening_hours' ] ) ) {
								echo isset( $theme_options[ 'header_opening_hours_label' ] ) ? esc_html( $theme_options[ 'header_opening_hours_label' ] ) : esc_html__( 'Opening Hours', 'framework' ) ;
								echo ' : ';
								echo '<span>' . esc_html( $theme_options[ 'header_opening_hours' ] ) . '</span>';
							}

							if ( isset( $theme_options[ 'header_contact_number' ] ) && ! empty( $theme_options[ 'header_contact_number' ] ) ) {
								echo '<br class="visible-xs" />';
								echo '&nbsp;&nbsp;';
								echo isset( $theme_options[ 'header_contact_number_label' ] ) ? esc_html( $theme_options[ 'header_contact_number_label' ] ) : esc_html__( 'Contact', 'framework' ) ;
								echo ' : ';
								echo '<span>' . esc_html( $theme_options[ 'header_contact_number' ] ) . '</span>';
							}
							?>
						</p>
					</div>
			...
			<div class="row">
				<div class="<?php bc_all('12'); ?>">
					...
					<div class="logo clearfix">
						<?php
						$logo_path        = isset( $theme_options['website_logo']['url'] ) ? esc_url( $theme_options['website_logo']['url'] ) : '';
						$retina_logo_path = isset( $theme_options['website_retina_logo']['url'] ) ? esc_url( $theme_options['website_retina_logo']['url'] ) : '';
