...
if ( is_home() ) {
	...
	if ( ! empty( $blog_page_id ) ) {
		...
		$inspiry_page_title = esc_html__( 'Blog', 'framework' );
	}

} elseif ( is_404() ) {

	$inspiry_page_title = esc_html__( 'Page 404', 'framework' );

} elseif ( is_search() ) {

	$inspiry_page_title = sprintf( esc_html__( 'Search Results for: %s', 'framework' ), get_search_query() );
	...
	if ( function_exists( 'is_shop' ) ) {
		...
		if ( is_shop() ) {
			$inspiry_page_title = esc_html__( 'Shop', 'framework' );
		}
	}
}
