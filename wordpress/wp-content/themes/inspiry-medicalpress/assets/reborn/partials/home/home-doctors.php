    ...
    <div class="container">
		...
		if ( ( ! empty( $theme_options['home_doctors_title'] ) ) || ( ! empty( $theme_options['home_doctors_description'] ) ) ) : ?>
            <header class="home-section-header animated fadeInUp">
				...
				if ( ! empty( $theme_options['home_doctors_title'] ) ) :
					echo '<h2 class="home-section-title">' . wp_kses( $theme_options['home_doctors_title'], $allowed_html_tags ) . '</h2>';
				endif;

				if ( ! empty( $theme_options['home_doctors_description'] ) ) :
					echo '<p class="home-section-description">' . wp_kses( $theme_options['home_doctors_description'], $allowed_html_tags ) . '</p>';
				endif;
				?>
            </header>
		<?php endif; ?>
        <div class="row">
			...
			if ( $doctor_query->have_posts() ) :
				...
				nothing_found( esc_html__( 'No doctor found !', 'framework' ) );
			endif;
