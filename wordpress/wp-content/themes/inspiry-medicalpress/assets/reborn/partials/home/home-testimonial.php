		...
		if ( ( $theme_options['testimonials_variation'] == '1' ) && ( ( ! empty( $theme_options['home_testimonials_title'] ) ) || ( ! empty( $theme_options['home_testimonials_description'] ) ) ) ) :
            ...
            <header class="home-section-header animated fadeInUp">
				...
				if ( ! empty( $theme_options['home_testimonials_title'] ) ) :
					echo '<h2 class="home-section-title">' . wp_kses( $theme_options['home_testimonials_title'], $allowed_html_tags ) . '</h2>';
				endif;

				if ( ! empty( $theme_options['home_testimonials_description'] ) ) :
					echo '<p class="home-section-description">' . wp_kses( $theme_options['home_testimonials_description'], $allowed_html_tags ) . '</p>';
				endif;
				?>
            </header>
			<?php
                    ...
                    <div class="testimonials-carousel-item">
                        ...
                        <footer class="testimonials-carousel-item-footer clearfix">
                            ...
                            <div class="testimonial-author-content">
                                <h3 class="testimonial-author-name"><?php echo wp_kses( $testimonial_author_name, $allowed_html_tags ); ?></h3>
	                            <?php
	                            if ( ! empty( $testimonial_author_link ) && ! empty( $testimonial_author_organization ) ) : ?>
                                    <a class="testimonial-author-link" href="<?php echo esc_url( $testimonial_author_link ); ?>" target="_blank">
			                            <?php echo esc_html( $testimonial_author_organization ); ?>
                                    </a>
		                            <?php
	                            endif;
	                            ?>
