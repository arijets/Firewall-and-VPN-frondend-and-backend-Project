        ...
        <ul class="slides">
			...
			if ( ! empty( $gallery_images ) ) {
				foreach ( $gallery_images as $gallery_image ) {
					...
					echo '<li><a href="' . esc_url( $gallery_image['full_url'] ) . '" title="' . esc_html( $caption ) . '" >';
					echo '<img src="' . esc_url( $gallery_image['url'] ) . '" alt="' . esc_html( $gallery_image['title'] ) . '" />';
					echo '</a></li>';
				}
			} else if ( has_post_thumbnail( $post->ID ) ) {
...
if ( ! function_exists( 'get_default_banner' ) ) {
	...
	function get_default_banner() {
		...
		if ( ! empty( $theme_options['default_page_banner'] ) ) {
			$banner_image_path = esc_url( $theme_options['default_page_banner']['url'] );
		}
...
if ( ! function_exists( 'theme_breadcrumb' ) ) {
	...
	function theme_breadcrumb() {
		...
		if ( is_category() ) {
			...
			array_pop( $cats );
			foreach ( $cats as $cat ) {
				echo '<li>' . esc_html( $cat ) . '<span class="divider"></span></li>';
			}
		...
		if ( is_author() ) {
			echo '<li class="active">';
			esc_html_e( 'Author', 'framework' );
			echo '</li>';
		}

		if ( is_year() || is_month() || is_day() ) {
			echo '<li class="active">';
			esc_html_e( 'Archives', 'framework' );
			echo '</li>';
		}
		...
		if ( is_404() ) {
			echo '<li class="active">';
			esc_html_e( '404 - Page not Found', 'framework' );
			echo '</li>';
...
if ( ! function_exists( 'nothing_found' ) ) {
	...
	function nothing_found( $message ) {
        ...
        <div class="<?php bc_all( '12' ); ?>">
            <p class="nothing-found"><?php echo esc_html( $message ); ?></p>
        </div>
		<?php
	}
}
...
if ( ! function_exists( 'get_inspiry_filters' ) ) {
	function get_inspiry_filters( $term, $all_text = '', $isotope = true ) {
		...
		if ( ! $all_text ) {
			$all_text = esc_html__( 'All', 'framework' );
		}

		$html .= ' <li class="active"><a ' . esc_attr( $link_class ) . ' href="#" data-filter="*">' . esc_html( $all_text ) . '</a></li>';
		...
		if ( $term ) {
			...
			if ( ! empty( $terms ) ) {
				...
				foreach ( $terms as $term ) {
					...
					$html .= '<li><a ' . esc_attr( $link_class ) . ' href="' . get_term_link( $term->slug, $term->taxonomy ) . '" data-filter=".' . esc_attr( $term->slug ) . '">' . esc_html( $term->name ) . '</a></li>';
				}
			}
		}
	...
	function get_inspiry_social_nav( $nav_class = '', $display_option = '' ) {
		...
		if ( isset( $theme_options[ $display_option ] ) && '1' == $theme_options[ $display_option ] ) {
			...
			if ( ! empty( $theme_options['twitter_url'] ) ) {
				$html .= '<li><a class="twitter" target="_blank" href="' . esc_url( $theme_options['twitter_url'] ) . '"><i class="fab fa-twitter"></i></a></li>';
			}

			if ( ! empty( $theme_options['facebook_url'] ) ) {
				$html .= '<li><a class="facebook" target="_blank" href="' . esc_url( $theme_options['facebook_url'] ) . '"><i class="fab fa-facebook"></i></a></li>';
			}

			if ( ! empty( $theme_options['google_url'] ) ) {
				$html .= '<li><a class="google" target="_blank" href="' . esc_url( $theme_options['google_url'] ) . '"><i class="fab fa-google-plus-g"></i></a></li>';
			}

			if ( ! empty( $theme_options['linkedin_url'] ) ) {
				$html .= '<li><a class="linkedin" target="_blank" href="' . esc_url( $theme_options['linkedin_url'] ) . '"><i class="fab fa-linkedin-in"></i></a></li>';
			}

			if ( ! empty( $theme_options['instagram_url'] ) ) {
				$html .= '<li><a class="instagram" target="_blank" href="' . esc_url( $theme_options['instagram_url'] ) . '"><i class="fab fa-instagram"></i></a></li>';
			}

			if ( ! empty( $theme_options['youtube_url'] ) ) {
				$html .= '<li><a class="youtube" target="_blank" href="' . esc_url( $theme_options['youtube_url'] ) . '"><i class="fab fa-youtube"></i></a></li>';
			}

			if ( ! empty( $theme_options['skype_username'] ) ) {
				$html .= '<li><a class="skype" target="_blank" href="skype:' . esc_attr( $theme_options['skype_username'] ) . '?add"><i class="fab fa-skype"></i></a></li>';
			}

			if ( ! empty( $theme_options['rss_url'] ) ) {
				$html .= '<li><a class="rss" target="_blank" href="' . esc_url( $theme_options['rss_url'] ) . '"><i class="fas fa-rss"></i></a></li>';
			}
		...
		<div class="gdpr-checkbox-wrapper">
			<label for="<?php echo esc_attr( $form ) ?>-form-gdpr-checkbox">
				...
				<input id="<?php echo esc_attr( $form ) ?>-form-gdpr-checkbox"
					   class="required"
					   name="<?php echo esc_html( $form ) ?>-form-gdpr-checkbox"
					   type="checkbox"
