	...
	function send_message() {
		...
		if ( isset( $_POST['email'] ) ) {
			...
			if ( ! $to_email ) {
				echo json_encode( array(
					'success' => false,
					'message' => esc_html__( 'Target Email address is not properly configured!', 'framework' )
				) );
				die;
			}
			...
			$email_subject = esc_html__( 'New message sent by', 'framework' ) . ' ' . $from_name . ' ' . esc_html__( 'using contact form at', 'framework' ) . ' ' . get_bloginfo( 'name' );

			$email_body = esc_html__( "You have received a message from: ", 'framework' ) . $from_name . " <br/>";

			if ( ! empty( $phone_number ) ) {
				$email_body .= esc_html__( "Phone Number : ", 'framework' ) . $phone_number . " <br/>";
			}

			$email_body .= esc_html__( "Their additional message is as follows.", 'framework' ) . " <br/>";
			$email_body .= wpautop( $message ) . " <br/>";

//			if ( ! empty( $gdpr_agreement ) ) {
//				global $allowed_html_tags;
//				$email_body .= esc_html__( "GDPR Agreement:", 'framework' ) . " <br/>";
...
//			}
			...
			if ( wp_mail( $to_email, $email_subject, $email_body, $headers ) ) {
				echo json_encode( array(
					'success' => true,
					'message' => esc_html__( "Message Sent Successfully!", 'framework' )
				) );
			} else {
				echo json_encode( array(
						'success' => false,
						'message' => esc_html__( "Server Error: WordPress mail function failed!", 'framework' )
					)
				);
			}
...
if ( ! function_exists( 'make_appointment' ) ) {
	...
	function make_appointment() {
		...
		if ( isset( $_POST['email'] ) ):
			...
			$email_subject = esc_html__( 'New appointment request sent by', 'framework' ) . ' ' . $from_name . ' ' . __( 'using appointment form at', 'framework' ) . ' ' . get_bloginfo( 'name' );

			$email_body = esc_html__( "You have received an appointment request from: ", 'framework' ) . $from_name . " <br/>";
			...
			if ( ! empty( $appointment_date ) ) {
				$email_body .= esc_html__( "Appointment Date : ", 'framework' ) . $appointment_date . " <br/>";
			}
...
if ( ! function_exists( 'inspiry_appointment_confirmation_email' ) ) {
	...
	function inspiry_appointment_confirmation_email( $appointment_confirmation_args ) {
		...
		$email_subject = sprintf( esc_html__( 'Your appointment request has been received to  %1$s.', 'framework' ), $blog_name );
		...
		if ( ! empty( $email_confirmation_text ) ) {
			...
			$email_body = esc_html__( 'Dear ', 'framework' ) . $to_name . ',<br/><br/>';
			...
			$email_body .= esc_html__( 'Thank You!', 'framework' );
		}
