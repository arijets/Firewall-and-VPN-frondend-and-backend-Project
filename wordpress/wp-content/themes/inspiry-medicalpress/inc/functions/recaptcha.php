	...
	function inspiry_verify_google_recaptcha() {
		...
		if ( $resp->isSuccess() ) {
			...
			$error_messages      = array(
				'missing-input-secret'   => esc_html__( 'The secret parameter is missing.', 'framework' ) ,
				'invalid-input-secret'   => esc_html__( 'The secret parameter is invalid or malformed.', 'framework' ),
				'missing-input-response' => esc_html__( 'The response parameter is missing.', 'framework' ),
				'invalid-input-response' => esc_html__( 'The response parameter is invalid or malformed.', 'framework' ),
			);
			$error_codes         = $resp->getErrorCodes();
			$final_error_message = $error_messages[ $error_codes[0] ];
			echo json_encode( array(
				'success' => false,
				'message' => esc_html__( 'reCAPTCHA Failed:', 'framework' ) . ' ' . $final_error_message
			) );
			die;
		}
	}
