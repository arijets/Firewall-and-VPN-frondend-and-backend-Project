                ...
                <div class="<?php bc('12','12','12',''); ?>">
                    <div class="appointment-form clearfix appointment-shortcode">
                        <form class="row" id="appointment_form_three" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post">
                            ...
                            <div class=" <?php bc_all('6'); ?>">
                                <input type="text" name="number" id="app-number" placeholder="<?php esc_html_e('Phone Number', 'framework'); ?>" title="<?php esc_html_e('* Please provide a valid phone number.', 'framework'); ?>"/>
                            </div>
                            ...
                            <div class=" <?php bc_all('6'); ?>">
                                <input type="text" name="date" id="datepicker" placeholder="<?php esc_html_e('Appointment Date', 'framework'); ?>" title="<?php esc_html_e('* Please provide appointment date', 'framework'); ?>">
                            </div>
                            ...
                            <div class=" <?php bc_all('12'); ?>">
                                ...
                                <img src="<?php echo INSPIRY_ASSETS_DIR_URI; ?>/images/loader.gif" id="appointment-loader" alt="<?php esc_html_e('Loading...', 'framework'); ?>">
                ...
                if ($team_query->have_posts()) {
                    ...
                    while ($team_query->have_posts()) {
                        ...
                        ?> text-center doctor-wrapper">
                            ...
                            <a class="read-more" href="<?php the_permalink(); ?>"><?php esc_html_e('Read More', 'framework'); ?></a>
                        </div>
                        <?php
                        $loop_counter++;
                        if( ($loop_counter % 2) == 0 ){
...
if (!function_exists( 'inspiry_get_doctor' ) ) {
	function inspiry_get_doctor( $atts, $content = null ) {
		...
		if ( $doctor_query->have_posts() ) {
			...
			$msg = esc_html__( 'No Doctors found!', 'framework' );

			if ( $id ) {
				$msg .= sprintf( esc_html__( ' with ID: %s', 'framework' ), $id );
			} else if ( $name ) {
				$msg .= sprintf( esc_html__( ' with Name: %s', 'framework' ), $name );
			}
